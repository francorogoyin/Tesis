...
# GRAFICOS_BOXPLOT.PY
# ============================================================
# Módulo especializado en boxplots para visualización de
...
def Boxplot_Por_Categoria(
    ...
    """
    Crea boxplot de una variable por categoría.
    ...
    - Orden_Categorias: Orden de categorías (None usa
      ORDEN_CATEGORIAS).
    - Colores: Diccionario de colores por categoría (None
      usa COLORES_CATEGORIAS).
    - Escala_Y: Tupla (min, max) para eje Y.
    - Mostrar_Outliers: Si mostrar valores atípicos.
    ...
    if Titulo is None:
        Titulo = f'Distribución de {Variable} por Categoría'
    ...
    if Ruta_Salida is not None:
        ...
        print(f"  ✓ Boxplot guardado: {Ruta_Salida.name}")
    else:
        plt.show()
...
def Boxplot_Con_Puntos(
    ...
    Titulo: str = None,
    Orden_Categorias: List[str] = None,
    Alpha_Puntos: float = 0.3,
    Tamano_Puntos: int = 3,
    Ruta_Salida: Path = None
) -> None:

    """
    Crea boxplot con puntos individuales superpuestos
    (swarmplot).
    ...
    - Alpha_Puntos: Transparencia de puntos (0-1).
    - Tamano_Puntos: Tamaño de puntos.
...
def Boxplots_Multiples(
    ...
    """
    Crea múltiples boxplots en una cuadrícula.
    ...
    if Ruta_Salida is not None:
        ...
        print(
            f"  ✓ Boxplots múltiples guardados: "
            f"{Ruta_Salida.name}"
        )
    else:
        plt.show()
...
def Boxplot_Horizontal(
    ...
    """
    Crea boxplot horizontal (útil para muchas categorías).
    ...
    # Configurar.
    Ax.set_ylabel(Columna_Categoria, fontsize=12)
...
if __name__ == "__main__":
    print("Módulo de boxplots cargado.")